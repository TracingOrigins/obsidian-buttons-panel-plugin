...
- 🖱️ **编辑模式**：开启编辑模式后，支持在按钮面板上新建分类和按钮，支持分类和按钮右键菜单（移动、编辑、复制、删除）
...
- 您可以在面板设置中为边栏面板按钮启用"编辑模式"。
- **编辑模式下**，分类和按钮支持右键菜单操作，包括：
    - **移动**：进入分类或按钮的移动模式，快速移动分类或按钮的位置
    - **编辑**：快速进入分类或按钮的编辑界面
    - **复制**：将分类或按钮复制到同一分类末尾
    - **删除**：确认后移除分类或按钮
...
- **进入移动模式**：在编辑模式下，右键点击按钮，选择"移动"即可进入按钮移动模式。
- **移动方式**：
    - **点击其他按钮**：将按钮移动到该按钮的位置
    - **点击分类名称**：将按钮移动到该分类的最后一个位置（快速移动到分类末尾）
    - **点击按钮容器空白处**：将按钮移动到当前分类的最后一个位置
    - **点击空分类占位符**：将按钮移动到该分类的第一个位置
    - **点击自身**：退出移动模式，不改变位置
    - **按 ESC 键**：退出移动模式，不改变位置
- 移动完成后会自动保存并刷新面板。
