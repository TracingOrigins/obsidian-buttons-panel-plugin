 ...
 * 负责渲染和管理按钮设置界面，包括分类、按钮的增删改查、拖拽排序、移动、长按菜单等。
 * 支持移动端和桌面端的交互优化。
 */
export class ButtonManagementSection {
     ...
     * 构造函数，初始化插件、app、回调
     */
    constructor(plugin: ButtonsPanelPlugin, app: App, displayCallback?: () => void) {
        this.plugin = plugin;
        this.app = app;
    ...
    create(containerEl: HTMLElement): void {
        // 渲染每个分类
        const sortedCategories = [...this.plugin.settings.categories].sort(
            (a, b) => a.order - b.order
        );
    ...
    /**
     * 渲染单个按钮的设置项，包括编辑、复制、删除、拖拽等。
     * @param container 按钮列表容器
     * @param button 按钮配置
     * @param category 所属分类
     * @param buttonIndex 按钮在分类中的索引
     */
    private renderButtonSettingItem(
     ...
     * @param position 插入位置
     */
    private insertButtonToCategory(
        category: CategoryConfig,
        button: ButtonConfig,
        position: number
    ): void {
...
export function createButtonManagementSection(
    ...
    const section = new ButtonManagementSection(plugin, app, displayCallback);
    section.create(containerEl);
}
