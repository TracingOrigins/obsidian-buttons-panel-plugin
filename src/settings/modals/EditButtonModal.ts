...
import { ActionSequence } from '@/settings/actions/ActionSequence';
import { NameInput, IconInput } from '@/settings/components';

/**
 * EditButtonModal 编辑按钮模态框类。
...
export class EditButtonModal extends Modal {
    ...
    createBasicSettings(container: HTMLElement): void {
        ...
        this.nameInput = new NameInput(container, {
            name: t('button_name', this.plugin),
            description: t('button_name_desc', this.plugin),
            placeholder: t('button_name_placeholder', this.plugin),
        ...
        this.iconInput = new IconInput(
            container,
            {
                name: t('button_icon', this.plugin),
                description: t('button_icon_desc', this.plugin),
                placeholder: t('button_icon_placeholder', this.plugin),
                searchTooltip: t('search_icons_tooltip', this.plugin),
                uploadTooltip: t('upload_svg_icon_tooltip', this.plugin),
            },
    ...
    createActionSettings(container: HTMLElement): void {
        ...
        basicActionSettings.createEl('h4', { text: t('basic_settings', this.plugin) });
        // 执行模式
        new Setting(basicActionSettings)
            .setName(t('execution_mode', this.plugin))
            .setDesc(t('execution_mode_desc', this.plugin))
            .addDropdown((drop) => {
                drop.addOption('sequential', t('sequential', this.plugin));
                drop.addOption('parallel', t('parallel', this.plugin));
        ...
        const stopSetting = new Setting(basicActionSettings)
            .setName(t('stop_on_error', this.plugin))
            .setDesc(t('stop_on_error_desc', this.plugin))
            .addToggle((toggle) => {
        ...
        if (isParallel) {
            ...
            stopSetting.setDesc(t('only_sequential_effective', this.plugin));
        }
        // 动作间延迟
        const delaySetting = new Setting(basicActionSettings)
            .setName(t('delay_between_actions', this.plugin))
            .setDesc(t('delay_between_actions_desc', this.plugin))
            .addText((text) => {
        ...
        if (isParallel) {
            ...
            delaySetting.setDesc(t('only_sequential_effective', this.plugin));
        }
    ...
    private createActionButtons(container: HTMLElement): void {
        new Setting(container)
            .addButton((btn) => {
                btn.setButtonText(t('save', this.plugin))
                    .setCta()
                    .setClass('custom-save-button')
                    .onClick(() => this.saveButton());
            })
            .addButton((btn) => {
                btn.setButtonText(t('cancel', this.plugin))
    ...
    async saveButton(): Promise<void> {
        ...
        if (!this.nameInput?.getValue()?.trim()) {
            this.nameInput?.setError(t('please_complete_required_fields', this.plugin));
            hasError = true;
        } else {
        ...
        if (!this.actionSequence.validateAll()) {
            this.actionSequence.setAllErrors(t('please_complete_required_fields', this.plugin));
            hasError = true;
        } else {
        ...
        // 如果有错误，显示通知并返回
        if (hasError) {
            new Notice(t('please_complete_required_fields', this.plugin));
            return;
        }
