  ...
  onOpen(): void {
    ...
    const renderSuggestions = () => {
      ...
      if (filteredFolders.length === 0) {
        suggestions.createDiv({ text: t('no_folders_available', this.plugin), cls: 'suggestion-item' });
        return;
      }
    ...
    input.inputEl.addEventListener('keydown', (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'ArrowDown':
          e.preventDefault();
          selectedSuggestionIndex = (selectedSuggestionIndex + 1) % filteredFolders.length;
          renderSuggestions();
          break;
        case 'ArrowUp':
          e.preventDefault();
          selectedSuggestionIndex = (selectedSuggestionIndex - 1 + filteredFolders.length) % filteredFolders.length;
          renderSuggestions();
          break;
        case 'Enter':
          e.preventDefault();
