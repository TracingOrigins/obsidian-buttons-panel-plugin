...
export class FileSearchModal extends Modal {
   ...
   */
  constructor(app: App, plugin: ButtonsPanelPlugin, onSelect: (file: any) => void, options?: { rootFolder?: string, fileExts?: string[], showFileNameOnly?: boolean }) {
    super(app);
    this.plugin = plugin;
  ...
  onOpen(): void {
    ...
    const scrollToSelected = () => {
      const activeItem = suggestions.querySelector('.suggestion-item.is-active') as HTMLElement;
      if (activeItem) {
    ...
    const renderSuggestions = () => {
      ...
      if (filteredFiles.length === 0) {
        suggestions.createDiv({ text: t('no_files_available', this.plugin), cls: 'suggestion-item' });
        return;
      }
    ...
    input.inputEl.addEventListener('keydown', (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowUp':
          e.preventDefault();
          selectedSuggestionIndex = (selectedSuggestionIndex - 1 + filteredFiles.length) % filteredFiles.length;
          renderSuggestions();
          break;
        case 'Enter':
          e.preventDefault();
