  ...
  onOpen(): void {
    ...
    const renderSuggestions = () => {
      ...
      if (filteredFormats.length === 0) {
        suggestions.createDiv({ text: t('no_format_match', this.plugin), cls: 'suggestion-item' });
        return;
      }
    ...
    input.inputEl.addEventListener('keydown', (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case 'ArrowDown':
          e.preventDefault();
          selectedSuggestionIndex = (selectedSuggestionIndex + 1) % filteredFormats.length;
          renderSuggestions();
          break;
        case 'ArrowUp':
          e.preventDefault();
          selectedSuggestionIndex = (selectedSuggestionIndex - 1 + filteredFormats.length) % filteredFormats.length;
          renderSuggestions();
          break;
        case 'Enter':
          e.preventDefault();
