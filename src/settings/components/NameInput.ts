 ...
 *
 * // 在动作类中使用
 * class FileAction implements IButtonAction {
 *   private nameInput: NameInput;
 *
 *   render(container: HTMLElement): void {
 *     const nameContainer = container.createDiv();
 *     this.nameInput = new NameInput(nameContainer, {
 *       placeholder: '输入文件名',
 ...
 *         console.warn('文件名验证错误:', error);
 *       }
 *     });
 *   }
 *
 *   validate(): boolean {
 *     return this.nameInput && !this.nameInput.getValue().trim() === '';
 *   }
 * }
 */
