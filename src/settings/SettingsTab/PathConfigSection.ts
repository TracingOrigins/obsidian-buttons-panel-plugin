...
async function createPathsOnly(plugin: ButtonsPanelPlugin, app: App, templateInput: TextComponent, scriptInput: TextComponent): Promise<void> {
	...
	if (cleanTemplatePath) {
		...
		if (!templateFolder) {
			try {
				...
				new Notice(t('create_folder_failed', plugin) + `: ${cleanTemplatePath} - ${error.message}`);
				return;
			}
		}
	}
	...
	if (cleanScriptPath) {
		...
		if (!scriptFolder) {
			try {
				...
				new Notice(t('create_folder_failed', plugin) + `: ${cleanScriptPath} - ${error.message}`);
				return;
			}
		}
	}
	...
	if (scriptInput && scriptInput.inputEl) {
		const folder = cleanPath(scriptValue) ? app.vault.getAbstractFileByPath(cleanPath(scriptValue)) : null;
