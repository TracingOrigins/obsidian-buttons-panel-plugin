...
export class IconInputSuggest extends AbstractInputSuggest<string> {
    ...
    renderSuggestion(iconId: string, el: HTMLElement): void {
        el.addClass('buttons-panel-icon-suggestion');

        // 左侧图标预览
        const iconWrapper = el.createSpan({ cls: 'buttons-panel-icon-suggestion__icon' });
        if (iconId) {
        ...
        el.createSpan({
            text: iconId,
            cls: 'buttons-panel-icon-suggestion__label',
        });
    }
}
